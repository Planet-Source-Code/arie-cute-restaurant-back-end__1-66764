...
Begin VB.Form frmCManualKeluarDetail 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Inventory yang Keluar"
   ClientHeight    =   3690
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5775
   ...
   Begin VB.CommandButton cmdClose 
      ...
      Height          =   795
      Left            =   2940
      Picture         =   "frmCManualKeluarDetail.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   2820
      Width           =   795
   ...
   Begin VB.CommandButton cmdUpdate 
      ...
      Height          =   795
      Left            =   2100
      Picture         =   "frmCManualKeluarDetail.frx":030A
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   2820
      Width           =   795
   ...
   Begin VB.Image Image2 
      Height          =   570
      Left            =   120
      Picture         =   "frmCManualKeluarDetail.frx":0614
      Stretch         =   -1  'True
      Top             =   60
      Width           =   525
   End
   ...
   End
End
Attribute VB_Name = "frmCManualKeluarDetail"
...
Dim Batas, CompStockKeluar As Single
...
Private Sub cmdUpdate_Click()
  ...
  If dcInventory.Text = "" Then
    MsgBox "Data Inventory yang keluar tidak boleh kosong", vbCritical
    dcInventory.SetFocus
    Exit Sub
  End If
  iCounter = JumlahRecord("Select * From [C_IStockCard] Where Inventory = '" & dcInventory.BoundText & "' And PI = '" & frmCManualKeluar.txtFields(0).Text & "'", db)
  If iCounter > 0 Then
  ...
  rsKembaliInventory!Tanggal = frmCManualKeluar.txtFields(1).Text
  ...
  rsKembaliInventory!PI = frmCManualKeluar.txtFields(0).Text
  rsKembaliInventory!Keterangan = "Inventory Adjustment (Decrease)"
  rsKembaliInventory!UnitKeluar = (txtFields(0).Text * iFSatuanKecil) + txtFields(3).Text
  ...
  rsHistoryDetail!KodeRef = frmCManualKeluar.txtFields(0).Text
  ...
  '
  CompStockKeluar = (txtFields(0).Text * Konversi) + txtFields(3).Text
  '
  If bolResep Then
  ...
  Set rsAlert = Nothing
  '
  frmCManualKeluar.txtFields(7).Text = CCur(frmCManualKeluar.txtFields(7).Text) + CCur(txtFields(2).Text)
  ...
  If KeyAscii = 13 Then
    ...
    If rsDetailInventory.RecordCount <> 0 Then
      ...
      If Not bolResep Then
        Batas = rsDetailInventory!QtyOnHand
        With txtFields(3)
          .TabStop = True
          .Locked = False
          .BackColor = &HFFFFFF
        End With
      Else
        Batas = rsDetailInventory!QtyOnHandKecil
        With txtFields(3)
          .Text = 0
          .TabStop = False
          .Locked = True
  ...
  If KeyAscii = 13 Then
    Select Case Index
      Case 0
        ...
        If CSng(txtFields(0).Text) > Batas Then
          If Not bolResep Then
            MsgBox "Persediaan Inventory yang ada tinggal " & Batas & " " & sSatuanBesar, vbCritical
          Else
            MsgBox "Persediaan Inventory yang ada tinggal " & Batas & " " & sSatuanKecil, vbCritical
          End If
          txtFields(0).Text = 0
          dcInventory.SetFocus
          Exit Sub
