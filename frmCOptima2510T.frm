...
Begin VB.Form frmCOptima2510T 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Import Data dari Optima Quorion 2510T"
   ClientHeight    =   3630
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6330
   ...
   Begin VB.Frame fraFields 
      ...
      Begin VB.Image Image2 
         Height          =   570
         Left            =   180
         Picture         =   "frmCOptima2510T.frx":0000
         Stretch         =   -1  'True
         Top             =   60
         Width           =   525
      End
   ...
   End
End
Attribute VB_Name = "frmCOptima2510T"
...
Private Sub cmdBuka_Click()
  ...
  With frmUtama.dlgDialog
    .InitDir = sInitDir
    .DialogTitle = "Membuka File POS (Optima Quorion 2510T)"
...
Private Sub cmdImport_Click()
  ...
  Do While f.AtEndOfLine <> True
    '
    Baris = f.ReadLine
    Baris = Replace(Baris, "Rp", "")
    Baris = Replace(Baris, ")", "")
    Baris = Replace(Baris, "(", "")
    Baris = Replace(Baris, ".", "")
    Baris = Replace(Baris, Chr(34), "")
    Element = Split(Baris, ",")
    '
    If Left(Element(0), 3) = "000" Then
      sKodeMenu = Element(0)
      '
      Baris = f.ReadLine
      Baris = Replace(Baris, "Rp", "")
      ...
      '
      For iCounter = 0 To 6
        If IsNumeric(Element(iCounter)) Then
          iTerjual = Element(iCounter + 1)
          Exit For
        End If
      Next
      '
...
Private Sub txtMesin_KeyPress(KeyAscii As Integer)
  '
  If KeyAscii = 13 Then
    sKontrolAktif = "AIDM_2"
    '
    Dim rsOptima As New ADODB.Recordset
    rsOptima.Open "Select * From [C_Mesin] Where Mesin='" & txtMesin.Text & "' And TipeMesin='CR 2510T'", db, adOpenStatic, adLockReadOnly
    If rsOptima.RecordCount = 0 Then
      frmCari.Show vbModal
      txtMesin.SetFocus
    Else
      sInitDir = rsOptima!PathData
      txtFileName.Text = ""
      SendKeys "{TAB}"
    End If
    rsOptima.Close
